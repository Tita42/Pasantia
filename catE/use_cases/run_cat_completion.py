...
import click as ck
import os
from src.cat_completion import CatCompletion
...
@ck.command()
@ck.option('--use_case', '-case', required=True, type=ck.Choice(["go", "foodon", "ontologia"]))
@ck.option('--el', '-el', is_flag=True)
...
def main(use_case, el, emb_dim, batch_size, lr, num_negs, margin,
    ...
    if not not_sweep:
        # Si estás usando un sweep, WandB debería tener los valores configurados
        emb_dim = wandb.config.get("emb_dim", 256)
        batch_size = wandb.config.get("batch_size", 4096*8)
        lr = wandb.config.get("lr", 0.0001)
        num_negs = wandb.config.get("num_negs", 4)
        margin = wandb.config.get("margin", 0.1)
        loss_type = wandb.config.get("loss_type", "normal")
    else:
        # Configuración manual sin sweep
        wandb_logger.log({
            "emb_dim": emb_dim,
    ...
    root = os.path.abspath(f"{use_case}/data")
    ...
    validation_file = os.path.join(root, "valid.csv")
