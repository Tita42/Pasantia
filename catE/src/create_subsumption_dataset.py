...
def main(input_ontology, min_number, percentage, random_seed):
    ...
    manager.saveOntology(ontology, RDFXMLDocumentFormat(), IRI.create("file:/" + os.path.abspath(train_file_name).replace("\\", "/")))
    manager.saveOntology(valid_ontology, RDFXMLDocumentFormat(), IRI.create("file:/" + os.path.abspath(train_file_name).replace("\\", "/")))
    manager.saveOntology(test_ontology, RDFXMLDocumentFormat(), IRI.create("file:/" + os.path.abspath(train_file_name).replace("\\", "/")))
