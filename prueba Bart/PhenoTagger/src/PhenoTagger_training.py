...
@author: luol2
(Modificado para usar 'bert-base-cased' y diccionarios propios)
"""
...
def CNN_training(trainfiles, vocabfiles, modelfile, EPOCH=150):
    ...
    train_x, train_y = cnn_model.rep.represent_instances_all_feas(
        train_set, train_label,
        word_max_len=cnn_model.hyper['sen_max'],
        char_max_len=cnn_model.hyper['word_max'],
        training=True
    )
...
def BERT_training(trainfiles, vocabfiles, modelfile, EPOCH=100):
    ...
    trainfile = trainfiles['trainfile']
    train_set, train_label = ml_intext(trainfile)
    train_x, train_y = bert_model.rep.load_data(
        train_set, train_label,
        word_max_len=bert_model.maxlen,
        training=True
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument('--modeltype', '-m', help="deep learning model (cnn, bioformer, pubmedbert, biobert o bertbase?)", default='bertbase')
    ...
    if args.modeltype == 'cnn':
        vocabfiles = {
            'w2vfile': '../models/bio_embedding_intrinsic.d200',
            'charfile': '../dict/char.vocab',
            'labelfile': '../diccionarios/lable.vocab',
            'posfile': '../dict/pos.vocab'
        }
        trainfiles = {'trainfile': args.trainfile, 'devfile': args.devfile, 'devout': args.output + 'cnn_dev_temp_b64.tsv'}
        ...
        trainfiles = {'trainfile': args.trainfile, 'devfile': args.devfile, 'devout': args.output + 'bioformer_dev_temp.tsv'}
        ...
        trainfiles = {'trainfile': args.trainfile, 'devfile': args.devfile, 'devout': args.output + 'pubmedbert_dev_temp.tsv'}
        ...
        trainfiles = {'trainfile': args.trainfile, 'devfile': args.devfile, 'devout': args.output + 'biobert_dev_temp.tsv'}
        modelfile = args.output + 'biobert_PT_v1.2-new.h5'
        BERT_training(trainfiles, vocabfiles, modelfile)

    # === NUEVO: bert-base-cased (HF) ===
    elif args.modeltype == 'bertbase':
        vocabfiles = {
            'labelfile': '../diccionarios/lable.vocab',
            'checkpoint_path': 'bert-base-cased',
            'lowercase': False
        }
        trainfiles = {'trainfile': args.trainfile, 'devfile': args.devfile, 'devout': args.output + 'bertbase_dev_temp.tsv'}
        modelfile = args.output + 'bertbase_PT.h5'
        BERT_training(trainfiles, vocabfiles, modelfile)

    else:
        raise ValueError("modeltype debe ser uno de: cnn | bioformer | pubmedbert | biobert | bertbase")
